...
Run `./scripts/validate` from the repository root before opening a pull request. It uses the released `api-mcp` module in an ordinary checkout. For coordinated, unreleased engine work, set `API_MCP_SOURCE=/absolute/path/to/api-mcp`; validation then requires the exact clean commit recorded in `API_MCP_COMMIT` without adding a permanent local `replace` directive. Changes to the MCP engine belong in `cortexium-io/api-mcp`; update `API_MCP_VERSION` only after that engine version has been released.
